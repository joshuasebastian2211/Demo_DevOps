...
namespace SampleBoilerTemp.Tests
{
    public abstract class SampleBoilerTempTestBase : AbpIntegratedTestBase<SampleBoilerTempTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected SampleBoilerTempTestBase()
        {
        ...
        protected void UsingDbContext(Action<SampleBoilerTempDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SampleBoilerTempDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SampleBoilerTempDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SampleBoilerTempDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SampleBoilerTempDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SampleBoilerTempDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SampleBoilerTempDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SampleBoilerTempDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SampleBoilerTempDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SampleBoilerTempDbContext, Task<T>> func)
        {
            T result;
